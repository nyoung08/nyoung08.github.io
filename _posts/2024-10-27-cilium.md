...
tc(traffic control)의 ingress 훅에 연결된 bpf프로그램은 네트워크 인터페이스와 연결되지만, 네트워크 스택이 패킷을 초기에 처리한 이후 실행된다. bpf는 패킷의 메타데이터에는 접근이 가능하기 때문에 L3와 L4 엔드포인트 정책을 적용하거나 로컬 노드에서 트래픽을 특정 엔드포인트로 리다이렉트 시킬 수 있다. 그러나 L7 트래픽의 경우, 보다 복잡한 처리를 위해 envoy와 같은 프록시가 필요하기 때문에 어쩔 수 없이 iptables를 타게 된다. 전체 아키텍처를 보면 iptables 대신 커널에서 bpf 훅을 통해 트래픽을 제어하고, 기존의 conntrack 테이블 대신 bpf map에 기록이 남는다. 
